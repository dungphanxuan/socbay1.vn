...
class Star extends ActiveRecord
{
    ...
    public static function castStar($model, $userID, $starValue = -1)
    {
        ...
        Star::getFollowerCount($model, true);
        Star::getStarValue($model, $userID, true);
        return $star->star;
    }
    ...
    public static function getTargets($userID)
    {
        ...
        foreach (self::$modelClasses as $class) {
            ...
            $modelClass = "common\\models\\$class";
    ...
    {
        $class = $model->getObjectType();

        return User::find()->active()->andWhere([
            'id' => Star::find()
                ->select('user_id')
                ->where(['star' => 1, 'object_type' => $class, 'object_id' => $model->getObjectId()])
        ]);
    }
     ...
     * @return int
     */
    public static function getFollowerCount($model, $update = false)
    {
        //Todo set cache
        $key = ['StarCount', $model->getObjectType(), $model->id];

        $data = dataCache()->get($key);

        if ($data === false || $update) {
            $data = static::getFollowers($model)->count();

            dataCache()->set($key, $data, TimeHelper::SECONDS_IN_AN_HOUR);
        }
     ...
     * @return int
     */
    public static function getStarValue($model, $userID, $update = false)
    {
        $key = ['StarValue', $model->getObjectType(), $model->id, $userID];
     ...
     * @return int
     */
    public static function getStarValue1($model, $userID)
    {
        /** @var $star Star */
        $star = static::findOne([
            'object_type' => $model->getObjectType(),
