...
use frontend\modules\user\models\SetPasswordForm;
use Yii;
...
class SignInController extends \yii\web\Controller
{
     ...
     * @throws BadRequestHttpException
     */
    public function actionPhoneActivation($token)
    {
        $token = UserToken::find()
            ->byType(UserToken::TYPE_PHONE_ACTIVATION)
            ->byToken($token)
            ->notExpired()
            ->one();
        ...
        Yii::$app->getUser()->login($user);
        Yii::$app->getSession()->setFlash('alert', [
            'body' => Yii::t('frontend', 'Your account has been successfully activated phone number.'),
            'options' => ['class' => 'alert-success']
        ]);
    ...
    public function actionSetPassword()
    {
        ...
        return $this->render('setPasswordForm', [
            'model' => $model,
        ]);
    }
    ...
    public function successOAuthCallback($client)
    {
        ...
        //$needle = ['gmail.com'];
        $needles = [
            'oiqas.com', 'loaoa.com', 'mvrht.net', 'posdz.com', 'nezdiro.org', 'zhorachu.com',
            'datum2.com', 'tinoza.org', 'asorent.com', 'axsup.net', 'geronra.com', 'shitmail.me',
            'sqoai.com'
        ];
        ...
        if (!$user) {
            ...
            $user->valid_email = 1;
            $findByEmail = User::find()->where(['email' => $user->email])->one();
            if (!$user->email || $findByEmail) {
                $user->email = $randUser;
                $user->valid_email = 0;
            }
        ...
        if (Yii::$app->user->login($user, TimeHelper::SECONDS_IN_A_MONTH)) {
            return true;
        }
